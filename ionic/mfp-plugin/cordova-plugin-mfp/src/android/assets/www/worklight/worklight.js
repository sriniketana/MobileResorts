...
function WLResourceRequest(_url, _method, _options) {
    ...
    if (typeof(_options) === 'number') {
        ...
        if (!isUndefinedOrNull(_options) && !isUndefinedOrNull(_options.useAPIProxy)){
            useAPIProxy = _options.useAPIProxy;
        }
    }
    ...
    function buildRequestUrl() {
        ...
        if (url.indexOf('http:') >= 0 || url.indexOf('https:') >= 0) {
            /* Absolute URL - the API proxy needs a relative URL */
            if ( useAPIProxy === true ){
                dfd.reject('The useAPIProxy option can only be used with a relative URL ');
            }
            dfd.resolve(url);
        } else {
            WL.App.getServerUrl(
                function(serverUrl) {
                    ...
                    if (useAPIProxy) {
                        var apiProxyUrl = WL.Client.getAppProperty('API_PROXY_URL');
                        if (!apiProxyUrl){
